#!/usr/bin/env bash
#
# Generate a small CA + two server certs (hello.local, api.local) for
# the downstream-TLS demo. Files land in ./certs/. Idempotent: skips
# if the CA + both leaf certs already exist.
#
...
# and cert-manager (or another rotator) managing renewal. See the
# "Run with cert-manager" sidebar in the README.
...
if [[ -f ca.crt && -f hello.local.crt && -f api.local.crt ]]; then
...
rm -f ca.srl
echo "Generated CA + certs for hello.local + api.local in $CERTDIR"
